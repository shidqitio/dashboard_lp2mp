...
class ControllerPusjian extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->has('search') && $from != NULL && $to != NULL) {
                ...
                return view('users.pusjian.kalenderpjn', ['kalender' => $search]);
            ...
            $kalender = DB::table('kalender')
                ->select('*')
                ->get();
            return view('users.pusjian.kalenderpjn', ['kalender' => $kalender]);
        }
    }
